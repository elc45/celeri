...
1. Generate synthetic surface velocities
    ...
    - Run RBM estimation with real data
        - Make sure to save elastic kernels with `"save_elastic": 1,` setting in command file.  This will ensure that the elastic kernels are saved and don't have to be recomputed for every model run.  This is valid so long as the model geometry (including meshes) do not change.
